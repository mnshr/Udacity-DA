...
#### Occupation
```{r fig.height=9, fig.width=9, Univar_Occup}

Univar_Occup_plot <- ggplot(data=pd,aes(Occupation))+
  geom_bar() +
  scale_y_log10()
  
unique(pd$Occupation)

Univar_Occup_plot + coord_flip()
...
grid.arrange(ggplot(aes(x = EmploymentStatusDuration), 
       data = subset(pd, !is.na(EmploymentStatusDuration))) + 
  geom_histogram(binwidth=3, fill = '#5760AB'),
  ggplot(pd, aes( x = 1, y = EmploymentStatusDuration )) + 
              scale_y_log10()+
               geom_boxplot(alpha = 0.8, color = 'brown'), ncol=2)
...
From the box plot for EmploymentStatusDuration we don't see any unusual outlier.
...
The box plot shows a quite a few outliers having greater than 370-380 inquiries.
...
LoanStatusOrd=factor(pd$LoanStatus, 
                     levels=c("Current", "FinalPaymentInProgress", "Completed", 
                              "Cancelled", "Defaulted", "Chargedoff", 
                              "Past Due (1-15 days)", "Past Due (16-30 days)", 
                              "Past Due (31-60 days)","Past Due (61-90 days)", 
                            "Past Due (91-120 days)", "Past Due (>120 days)"))

LoanStatusOrdRev=factor(pd$LoanStatus, 
                levels=rev(c("Current", "FinalPaymentInProgress", "Completed", 
                              "Cancelled", "Defaulted", "Chargedoff", 
                              "Past Due (1-15 days)", "Past Due (16-30 days)", 
                              "Past Due (31-60 days)","Past Due (61-90 days)", 
                            "Past Due (91-120 days)", "Past Due (>120 days)")))
...
# Flipped the axes and reversed the order for LoanStatus
ggplot(subset(pd, !is.na(LoanStatus)),aes(x=LoanStatus))+
  geom_bar(fill=I('#78D20b'))+
  aes(x = LoanStatusOrdRev) +
  scale_y_log10()+
  coord_flip()
  theme(axis.text.x = element_text(face="bold",  
                           size=10, angle=60),
...
The box plot doesn't show any outliers, as expected.
...
ggplot(subset(pd, !is.na(CreditRange)),
       #aes(reorder(CreditRange, -table(CreditRange)[CreditRange]))) +
       aes(x=CreditRange)) +
  geom_bar(fill=I('#78D20b')) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
About 6% of loans are to borrowers with credit below 600 or no credit rating at all. Except the one outlier nearing 0-10, there is nothing unusual about the distribution in the box plot.
...
The graph shows an almost normal distribution and no outliers in the box plot.
...
ggplot(data = 
         subset(pd, ProsperCreditRating!="")) + 
  aes(x=ProsperCreditRating) +
  geom_bar(fill = '#78D20b') 
...
```{r fig.width=10, fig.height=10, Bivar_corr}
...
```{r fig.width=10, fig.height=10, Bivar_corr2}
...
```{r fig.height=6, fig.width=9, Bivar_Loan}
...
ggplot(data = subset(pd, ProsperCreditRating!=""), 
...
ggplot(data=subset(pd, !is.na(Occupation_new)),
  ...
  scale_y_log10() +
  theme(axis.text.x = 
          element_text(size=10, angle = 60, hjust = 1),
...
```{r Bivar_Occup}
ggplot(data=subset(pd, !is.na(Occupation_new)),
       aes(x=Occupation_new,y=MeanCreditScore))+
  geom_boxplot(aes(colour=Occupation_new)) +
  theme(axis.text.x = 
          element_text(size=10, angle = 60, hjust = 1),
        legend.position="none") +
...
```{r Bivar_Rating}
ggplot(data=subset(pd, ProsperCreditRating!=""),
       aes(x=ProsperCreditRating,y=MeanCreditScore))+
...
ggplot(data = subset(pd, !is.na(IsBorrowerHomeowner)), 
  aes(x = as.factor(IsBorrowerHomeowner), y = MeanCreditScore)) +
  geom_boxplot(fill = '#78D20b') + 
  scale_y_continuous(limits = c(quantile(pd$MeanCreditScore, 0.05, na.rm=T), 
                                quantile(pd$MeanCreditScore, 0.95, na.rm=T)))
```
...
ggplot(data = subset(pd, !is.na(LoanStatus)), 
       aes(x = LoanStatusOrd, y = MeanCreditScore)) +
...
```{r Bivar_Inc}
ggplot(data = subset(pd, !is.na(IncomeRange)), 
       aes(x = IncomeRangeOrd, y = MeanCreditScore, color=IncomeRangeOrd)) +
  geom_boxplot() + 
  scale_y_continuous(limits = c(quantile(pd$MeanCreditScore, 0.05, na.rm=T), 
                              quantile(pd$MeanCreditScore, 0.95, na.rm=T))) +
  theme(axis.text.x = 
...
```{r echo=FALSE, warning=FALSE, message=FALSE, cache=TRUE, cache.path = 'cache/', fig.path='figure/', Multivar_0}
ggplot(aes(x=factor(LoanStatus), 
           y= MeanCreditScore,Term, fill = factor(IsBorrowerHomeowner)), 
        data = subset(pd, ProsperCreditRating!="")) +
  coord_cartesian(ylim = c(quantile(pd$MeanCreditScore, 0.025,na.rm=T),
                           quantile(pd$MeanCreditScore, 0.975,na.rm=T))) +
  geom_boxplot( alpha = .5) +
theme(axis.text.x = element_text(size=10, angle = 60, hjust = 1)) 
```

The plot above shows the relationship among MeanCreditScore, LoanStatus and home ownership. 
It is clearly evident that in most of the cases the non-owners of home have a lower credit score, except when the loan status was "Cancelled". And the highest scorers are the ones who have have completed their loans and are also home owners, although those with "Current" status are close followers.
...
pdncr=subset(pd, ProsperCreditRating!="")
ggplot(data = pdncr, 
       aes(x = pdncr$LoanMonthsSinceOrigination, y = pdncr$MeanCreditScore)) +
  coord_cartesian(xlim = 
                  c(quantile(pdncr$LoanMonthsSinceOrigination, 0.075, na.rm=T), 
                  quantile(pdncr$LoanMonthsSinceOrigination, 0.925, na.rm=T)),
                  ylim = 
                    c(quantile(pdncr$MeanCreditScore, 0.075,na.rm=T),
                    quantile(pdncr$MeanCreditScore, 0.925,na.rm=T))) +
  geom_smooth(aes(color=ProsperCreditRating)) + 
...
p11 <- ggplot(data = subset(pd, !is.na(Term)), 
  ...
  xlab("Term [months]") +
  guides(fill=FALSE) +
  scale_color_brewer(palette="RdBu") #GnBU

p12 <- ggplot(data = subset(pd, ProsperCreditRating!=""), 
  aes(x = as.factor(ProsperCreditRating), y = MeanCreditScore, 
      color=ProsperCreditRating), legend.position="none")+
...
### Plot Two
```{r Plot_Two, fig.height=10, fig.width=10}
secPalette <- c("#000000", "#0000FF", "#00FF00", "#FF0000", "#00FFFF", 
                "#FFFF00", "#FF00FF", "#FFFFFF")

p21 <- ggplot(aes(x=BorrowerRate, y=MeanCreditScore), 
       data=subset(pd, !is.na(BorrowerRate))) + 
  geom_jitter(alpha=0.20, aes(color=IncomeRangeOrd)) +
  scale_colour_manual(values=secPalette) +
  geom_smooth(aes(color=IncomeRangeOrd), se=FALSE) +
  ylab("Credit Score") +
  xlab("Borrower Rate [%]") +
...
### Plot Three
```{r Plot_Three, fig.height=9, fig.width=11}
# Subset the data with for non-null credit ratings
#ndpd=subset(pd, !is.null(pd$ProsperCreditRating))
ndpd=subset(subset(pd, ProsperCreditRating!=""), 
            IncomeRange != "Not displayed")
ggplot(data = ndpd, 
       aes(x = ndpd$LoanMonthsSinceOrigination, y = ndpd$MeanCreditScore)) +
  coord_cartesian(xlim = 
                    c(quantile(ndpd$LoanMonthsSinceOrigination, 0.075, na.rm=T), 
                    quantile(ndpd$LoanMonthsSinceOrigination, 0.925, na.rm=T)),
                  ylim = 
                    c(quantile(ndpd$MeanCreditScore, 0.075,na.rm=T),
                    quantile(ndpd$MeanCreditScore, 0.925,na.rm=T))) +
  #scale_colour_manual(palette="spectral") +
  scale_colour_brewer(palette = "Spectral") +
  geom_smooth(aes(color=ProsperCreditRating), se=FALSE) + 
  facet_wrap(~IncomeRange) +
  ylab("Credit Score") +
  xlab("Loan Age [Months]") +
ggtitle("Credit Scores by Loan Age for difft Credit Ratings and Income Ranges")
